...
import static org.grits.toolbox.tools.glycanbuilder.core.renderer.utils.Geometry.*;
 ...
 * {@link Linkage} object given the current graphic options
 * ({@link GraphicOptionsSWT}. The rules to draw the linkage in the different
 * notations are stored in the {@link LinkageStyleDictionary}.
 * 
 * @author Alessio Ceroni (a.ceroni@imperial.ac.uk)
 * @author Masaaki Matsubara (matsubara@uga.edu)
 */

public class LinkageRendererSWT extends SWTRenderer {
	...
	/**
	 * Draw the line part of a linkage on a graphic context using the specified
	 * bounding boxes.
	 * 
	 * @param gc
	 *            the graphic context
	 ...
	 * @param parent_border_bbox
	 *            the bounding box of the parent residue including the residues on
	 *            border
	 * @param child_bbox
	 *            the bounding box of the child residue
	 * @param child_border_bbox
	 *            the bounding box of the child residue including the residues on
	 *            border
	 */
	public void paintEdge(GC gc, Linkage link, boolean selected, Rectangle parent_bbox, Rectangle parent_border_bbox,
			Rectangle child_bbox, Rectangle child_border_bbox) {
		// public void paintEdge(Graphics2D g2d, Linkage link, boolean selected,
		// Rectangle parent_bbox, Rectangle parent_border_bbox, Rectangle child_bbox,
		// Rectangle child_border_bbox) {

		if (link == null)
			return;

		LineAttributes edge_stroke = createStroke(link, selected);
		// Stroke edge_stroke = createStroke(link, selected);
		Path edge_shape = createShape(gc.getDevice(), link, parent_bbox, child_bbox);
		...
		if (edge_shape != null) {
			...
			gc.setLineAttributes(new LineAttributes(1.f));
	...
	/**
	 * Draw the text part of a linkage on a graphic context using the specified
	 * bounding boxes.
	 * 
	 * @param gc
	 *            the graphic context
	 ...
	 *            border
	 */
	public void paintInfo(GC gc, Linkage link, Rectangle parent_bbox, Rectangle parent_border_bbox,
			Rectangle child_bbox, Rectangle child_border_bbox) {
		// public void paintInfo(Graphics2D g2d, Linkage link, Rectangle parent_bbox,
		// Rectangle parent_border_bbox, Rectangle child_bbox, Rectangle
		// child_border_bbox) {
		...
		Font new_font = new Font(gc.getDevice(), theGraphicOptions.LINKAGE_INFO_FONT_FACE,
				theGraphicOptions.LINKAGE_INFO_SIZE, SWT.NORMAL);
		// Font new_font = new Font(theGraphicOptions.LINKAGE_INFO_FONT_FACE,
		// Font.PLAIN,
		...
		if (style.showParentLinkage(link))
			paintInfo(gc, link.getParentPositionsString(), parent_bbox, parent_border_bbox, child_bbox,
					child_border_bbox, true, false, link.hasMultipleBonds());
		if (style.showAnomericCarbon(link))
			paintInfo(gc, link.getChildPositionsString(), parent_bbox, parent_border_bbox, child_bbox,
					child_border_bbox, false, true, link.hasMultipleBonds());
		if (style.showAnomericState(link, child.getAnomericState()))
			paintInfo(gc, TextUtils.toGreek(child.getAnomericState()), parent_bbox, parent_border_bbox, child_bbox,
	...
	private void paintInfo(GC gc, String text, Rectangle p, Rectangle pb, Rectangle c, Rectangle cb, boolean toparent,
			boolean above, boolean multiple) {
		// private void paintInfo(Graphics2D g2d, String text, Rectangle p, Rectangle
		// pb, Rectangle c, Rectangle cb, boolean toparent, boolean above, boolean
		// multiple) {
		...
		// g2d.clearRect(pos.x, (int) (pos.y - tb.getHeight()), (int) tb.getWidth(),
		// (int) tb.getHeight());
	...
	private Point computePosition(Rectangle tb, Rectangle p, Rectangle pb, Rectangle c, Rectangle cb, boolean toparent,
			boolean above, boolean multiple) {
		// private Point computePosition(Dimension tb, Rectangle p, Rectangle pb,
		// Rectangle c, Rectangle cb, boolean toparent,
		// boolean above, boolean multiple) {
		Point cp = center(p);
		Point cc = center(c);

//		double r = 0.5 * theGraphicOptions.LINKAGE_INFO_SIZE;
		double r = 0.5 * TextShapeUtils.getFontSizePixel(theGraphicOptions.LINKAGE_INFO_SIZE);
		...
		/*
		 * double pangle = angle(cc,cp); boolean add = (pangle>-Math.PI/2. &&
		 * pangle<Math.PI/2.); if( !above ) add = !add;
		 */
	...
	private LineAttributes createStroke(Linkage link, boolean selected) {
		...
		float fPattern = (float) (5.f * theGraphicOptions.SCALE);
		if (style.isDashed()) {
			return new LineAttributes((selected) ? 2.f : 1.f, SWT.CAP_FLAT, SWT.JOIN_ROUND, SWT.LINE_CUSTOM,
					new float[] { fPattern, fPattern }, 0.f, 1.f);
			// float[] dashes = { 5.f, 5.f };
			// return new BasicStroke((selected) ? 2.f : 1.f, BasicStroke.CAP_BUTT,
			// BasicStroke.JOIN_ROUND, 1.f, dashes,
			// 0.f);
		}

		return new LineAttributes((selected) ? 2.f : 1.f);
		// return new BasicStroke((selected) ? 2.f : 1.f);
	}

	static private Path createLine(Device device, Point p1, Point p2, boolean multiple) {
		// static private Shape createLine(Point p1, Point p2, boolean multiple) {
		if (!multiple)
			return createLine(device, p1, p2);

		Path gp = new Path(device);
		// GeneralPath gp = new GeneralPath();
		double a = angle(p1, p2);

		double tx = 2. * Math.cos(a + Math.PI / 2);
		double ty = 2. * Math.sin(a + Math.PI / 2);
		Path line1 = createLine(device, translate(p1, tx, ty), translate(p2, tx, ty), false);
		gp.addPath(line1);
		line1.dispose();
		// Shape line1 = createLine(translate(p1, 2. * Math.cos(a + Math.PI / 2), 2. *
		// Math.sin(a + Math.PI / 2)),
		// translate(p2, 2. * Math.cos(a + Math.PI / 2), 2. * Math.sin(a + Math.PI /
		// 2)), false);
		// gp.append(line1, false);

		tx = 2. * Math.cos(a - Math.PI / 2);
		ty = 2. * Math.sin(a - Math.PI / 2);
		Path line2 = createLine(device, translate(p1, tx, ty), translate(p2, tx, ty), false);
		gp.addPath(line2);
		line2.dispose();
		// Shape line2 = createLine(translate(p1, 2. * Math.cos(a - Math.PI / 2), 2. *
		// Math.sin(a - Math.PI / 2)),
		// translate(p2, 2. * Math.cos(a - Math.PI / 2), 2. * Math.sin(a - Math.PI /
		// 2)), false);
	...
	static private Path createLine(Device device, Point p1, Point p2) {
	...
	static private Path createCurve(Device device, Point p1, Point p2) {
	...
	static private Path createCurve(Device device, Point p1, Point p2, boolean multiple) {
		// static private Shape createCurve(Point p1, Point p2, boolean multiple) {
		if (!multiple)
			return createCurve(device, p1, p2);
		...
		double tx = 2. * Math.cos(a + Math.PI / 2);
		double ty = 2. * Math.sin(a + Math.PI / 2);
		Path curve1 = createCurve(device, translate(p1, tx, ty), translate(p2, tx, ty), false);
		gp.addPath(curve1);
		curve1.dispose();
		// Shape curve1 = createCurve(translate(p1, 2. * Math.cos(a + Math.PI / 2), 2. *
		// Math.sin(a + Math.PI / 2)),
		// translate(p2, 2. * Math.cos(a + Math.PI / 2), 2. * Math.sin(a + Math.PI /
		// 2)), false);
		...
		tx = 2. * Math.cos(a - Math.PI / 2);
		ty = 2. * Math.sin(a - Math.PI / 2);
		Path curve2 = createCurve(device, translate(p1, tx, ty), translate(p2, tx, ty), false);
		gp.addPath(curve2);
		curve2.dispose();
		// Shape curve2 = createCurve(translate(p1, 2. * Math.cos(a - Math.PI / 2), 2. *
		// Math.sin(a - Math.PI / 2)),
		// translate(p2, 2. * Math.cos(a - Math.PI / 2), 2. * Math.sin(a - Math.PI /
		// 2)), false);
	...
	private Path createShape(Device device, Linkage link, Rectangle parent_bbox, Rectangle child_bbox) {
		// private Shape createShape(Linkage link, Rectangle parent_bbox, Rectangle
		// child_bbox) {
		LinkageStyle style = theLinkageStyleDictionary.getStyle(link);
		...
		if (edge_style.equals("line"))
			return createLine(device, parent_center, child_center, link.hasMultipleBonds());
		// return createLine(parent_center, child_center, link.hasMultipleBonds());
		if (edge_style.equals("curve"))
			return createCurve(device, parent_center, child_center, link.hasMultipleBonds());
		// return createCurve(parent_center, child_center, link.hasMultipleBonds());

		return createLine(device, parent_center, child_center);
