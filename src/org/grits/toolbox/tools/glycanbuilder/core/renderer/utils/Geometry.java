...
public class Geometry {
	...
	/**
	 * Return the distance between the point <code>p</code> and the line passing by
	 * the points <code>s1</code> and <code>s2</code>.
	 */
	static public double distance(Point p, Point s1, Point s2) {

		double u = (double) ((p.x - s1.x) * (s2.x - s1.x) + (p.y - s1.y) * (s2.y - s1.y))
				/ (double) ((s2.x - s1.x) * (s2.x - s1.x) + (s2.y - s1.y) * (s2.y - s1.y));
		if (u < 0)
	...
	/**
	 * Return <code>true</code> if the first rectangle is on the left of the second.
	 */
...
/*	static public Insets getInsets(Rectangle in, Rectangle out) {
		in.
		return new Insets(top(in) - top(out), left(in) - left(out), bottom(out) - bottom(in), right(out) - right(in));
	}
*/
	static public double getExclusionRadius(Point center, double angle, Rectangle bbox) {
	...
	static public boolean overlapx(Rectangle a, Rectangle b, int toll) {
		if (a == null || b == null)
			return false;
	...
	static public boolean overlapy(Rectangle a, Rectangle b, int toll) {
		if (a == null || b == null)
			return false;
