...
/**
 * Utility class containing functions to export glycan structures and other
 * renderable objects into graphic formats.
 * 
...
public class SVGUtils {
	...
	/**
	 * Interface that must be implemented by an objects to be rendered as an image
	 * and exported into a graphic format.
	 */
	...
	/**
	 * Return a map containing all the supported graphical formats. The map is
	 * composed by pairs containing the identifier and the description of the
	 * format.
	 */
	...
	/**
	 * Return <code>true</code> if the identifier represent a supported graphical
	 * format.
	 */
	...
	/**
	 * Return a representation of a set of glycan structure as a string in SVG
	 * format.
	 * 
	 * @param gr
	 *            the GlycanRenderer used to render the structures
	 * @param structures
	 *            the structures to be rendered
	 */
	 ...
	 *            the structures to be rendered
	 * @param show_masses
	 *            <code>true</code> if the mass information should be included in
	 *            the graphical representation
	 * @param show_redend
	 *            <code>true</code> if the reducing end marker should be included in
	 *            the graphical representation
	 */
	static public String getVectorGraphics(GlycanRendererAWT gr, Collection<Glycan> structures, boolean show_masses,
			boolean show_redend) {
		if (structures == null)
		...
		try {
			...
			Rectangle all_bbox = sgr.computeBoundingBoxes(structures, show_masses, show_redend, posManager,
					bboxManager);
	...
	/**
	 * Return a representation of a set of glycan structure as an array of bytes in
	 * PDF format
	 * 
	 * @param gr
	 *            the GlycanRenderer used to render the structures
	 ...
	 *            the structures to be rendered
	 */
//	static public byte[] getPDFGraphics(GlycanRendererAWT gr, Collection<Glycan> structures) {
...
//	}
	 ...
	 *            the graphical representation
	 */
//	static public byte[] getPDFGraphics(GlycanRendererAWT gr, Collection<Glycan> structures, boolean show_masses,
//			boolean show_redend) {
...
//	}
	 ...
	 *            the structures to be rendered
	 */
	static public byte[] getPSGraphics(GlycanRendererAWT gr, Collection<Glycan> structures) {
	 ...
	 *            the graphical representation
	 */
	static public byte[] getPSGraphics(GlycanRendererAWT gr, Collection<Glycan> structures, boolean show_masses,
			boolean show_redend) {
	 ...
	 *            the structures to be rendered
	 */
	static public byte[] getEPSGraphics(GlycanRendererAWT gr, Collection<Glycan> structures) {
	 ...
	 *            the graphical representation
	 */
	static public byte[] getEPSGraphics(GlycanRendererAWT gr, Collection<Glycan> structures, boolean show_masses,
			boolean show_redend) {
	...
	static private org.apache.batik.svggen.SVGGraphics2D prepareGraphics(Dimension all_dim) {
		...
		/*
		 * double sf = Math.min(400./all_dim.width,400./all_dim.height); if( sf<1. )
		 * g2d.scale(sf,sf); else sf = 1.; all_dim.width = (int)(all_dim.width*sf);
	...
	static private byte[] transcode(org.apache.batik.svggen.SVGGraphics2D g2d, Dimension all_dim,
			org.apache.batik.transcoder.Transcoder transcoder) throws Exception {
		...
		transcoder.addTranscodingHint(org.apache.batik.transcoder.image.ImageTranscoder.KEY_BACKGROUND_COLOR,
				Color.white);
		transcoder.addTranscodingHint(org.apache.batik.transcoder.SVGAbstractTranscoder.KEY_PIXEL_UNIT_TO_MILLIMETER,
				new Float(0.3528f));
		transcoder.addTranscodingHint(org.apache.batik.transcoder.SVGAbstractTranscoder.KEY_MAX_WIDTH,
				new Float(all_dim.width));
		transcoder.addTranscodingHint(org.apache.batik.transcoder.SVGAbstractTranscoder.KEY_MAX_HEIGHT,
				new Float(all_dim.height));
		...
		transcoder.addTranscodingHint(org.apache.batik.transcoder.SVGAbstractTranscoder.KEY_HEIGHT,
				new Float(all_dim.height));
		transcoder.addTranscodingHint(org.apache.batik.transcoder.SVGAbstractTranscoder.KEY_AOI,
				new Rectangle(0, 0, all_dim.width, all_dim.height));
	...
	static private byte[] getTranscodedSVG(GlycanRendererAWT gr, Collection<Glycan> structures, boolean show_masses,
			boolean show_redend, org.apache.batik.transcoder.Transcoder transcoder) {
		if (structures == null)
			structures = new Vector<Glycan>();
	 ...
	 * 
	 * @param opaque
	 *            <code>false</code> if the background of the image must be
	 *            transparent
	 */
	...
	/*
	 * public byte[] getWMFGraphics(Collection<Glycan> structures) { try { //
	 * compute sizes PositionManager posManager = new PositionManager(); BBoxManager
	 * bboxManager = new BBoxManager(); Rectangle all_bbox =
	 * computeBoundingBoxes(structures,false,posManager,bboxManager); Dimension d =
	 * computeSize(all_bbox);
	 * 
	 * // Create an instance of the WMF writer WMF wmf = new WMF(); WMFGraphics2D
	 * g2d = new WMFGraphics2D(wmf, d.width, d.height);
	 * 
	 * // Render into the WMF Graphics2D HashSet<Residue> selected = new
	 * HashSet<Residue>();
	 * paintStructures(g2d,structures,selected,false,posManager,bboxManager);
	 * 
	 * // Stream out WMF int dpi = 100; ByteArrayOutputStream out = new
	 * ByteArrayOutputStream(); //wmf.writePlaceableWMF(out, 0, 0, d.width,
	 * d.height, dpi); wmf.writeWMF(out);
	 * 
	 * //FileOutputStream fos = new FileOutputStream("prova.wmf");
	 * //wmf.writePlaceableWMF(fos, 0, 0, d.width, d.height, dpi);
	 * //wmf.writeWMF(fos); //fos.close();
	 * 
	 * g2d.dispose(); return out.toByteArray(); } catch(Exception e) {
	 * LogUtils.report(e); return null; } }
	 */

	/**
	 * Export a representation of a set of glycan structure to a file in a certain
	 * graphical format.
	 * 
	 * @param gr
	 *            the GlycanRenderer used to render the structures
	 ...
	 *            the graphical representation
	 * @param format
	 *            the graphical format to be used
	 * @return <code>true</code> if the file was successfully created
	 */
	static public boolean export(GlycanRendererAWT gr, String filename, Collection<Glycan> structures,
			boolean show_masses, boolean show_redend, String format) {
	 ...
	 */
	static public boolean export(GlycanRendererAWT gr, String filename, Collection<Glycan> structures,
			boolean show_masses, boolean show_redend, double scale, String format) {
		try {
	...
	/**
	 * Create a representation of a set of glycan structure as an array of bytes in
	 ...
	 *            the graphical format to be used
	 */
	static public byte[] export(GlycanRendererAWT gr, Collection<Glycan> structures, boolean show_masses,
			boolean show_redend, String format) {
	 ...
	 */
	static public byte[] export(GlycanRendererAWT gr, Collection<Glycan> structures, boolean show_masses,
			boolean show_redend, double scale, String format) {
		try {
	...
	/**
	 * Export a representation of a set of glycan structure to a stream in a certain
	 * graphical format.
	 * 
	 * @param os
	 ...
	 */
	static public void export(OutputStream os, GlycanRendererAWT gr, Collection<Glycan> structures, boolean show_masses,
			boolean show_redend, String format) throws Exception {
	 ...
	 */
	static public void export(OutputStream os, GlycanRendererAWT gr, Collection<Glycan> structures, boolean show_masses,
			boolean show_redend, double scale, String format) throws Exception {
...
//			os.write(getPDFGraphics(gr, structures, show_masses, show_redend));
	...
	/**
	 * Export a representation of a Renderable object to a filename in a certain
	 * graphical format.
	 * 
	 * @param filename
	 *            the path to the destination file
	 * @param renderable
	 *            the renderable object to be exported
	 * @param format
	 *            the graphical format to be used
	 * @throws Exception
	 *             if the format is not supported
	...
	/**
	 * Export a representation of a Renderable object to a stream in a certain
	 * graphical format.
	 * 
	 * @param os
	 *            the destination output stream
