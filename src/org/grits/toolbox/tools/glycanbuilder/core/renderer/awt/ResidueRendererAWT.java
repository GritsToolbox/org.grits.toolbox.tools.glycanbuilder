 ...
 * ({@link GraphicOptions}. The rules to draw the residue in the different
...
public class ResidueRendererAWT {
	...
	/**
	 * Return a graphical representation of a residue type as an icon of
	 ...
	 * style in the current notation.
	 * 
	 * @param on_border <code>true</code> if the residue is displayed on the border
	 *                  of its parent, used for substitutions and modifications
	 */
	 ...
	 * @param selected    <code>true</code> if the residue should be shown as
	 *                    selected
	 * @param on_border   <code>true</code> if the residue should be drawn on the
	 ...
	 * @param sup_bbox    the bounding box used to decide the spatial orientation of
	 *                    the residue
	 * @param orientation the orientation of the residue
	 */
	public void paint(Graphics2D g2d, Residue node, boolean selected, boolean on_border, Rectangle par_bbox,
	 ...
	 * @param selected    <code>true</code> if the residue should be shown as
	 *                    selected
	 * @param active      <code>true</code> if the residue should be shown as active
	 * @param on_border   <code>true</code> if the residue should be drawn on the
	 *                    border of its parent
	 * @param par_bbox    the bounding box of the parent residue
	 * @param cur_bbox    the bounding box of the current residue
	 ...
	 * @param orientation the orientation of the residue
	 */
	public void paint(Graphics2D g2d, Residue node, boolean selected, boolean active, boolean on_border,
			Rectangle par_bbox, Rectangle cur_bbox, Rectangle sup_bbox, ResAngle orientation) {
		if (node == null)
			return;
		...
		if (shape == null || on_border || style.getText() != null) {
			...
			// set font
			// int font_size =
			// sat(9*cur_bbox.width/text.length()/10,theGraphicOptions.NODE_FONT_SIZE);
			...
			if (orientation.equals(0) || orientation.equals(180)) {
				Rectangle2D.Double text_rect = new Rectangle2D.Double(midx(cur_bbox) - text_bound.width / 2,
				...
				Rectangle2D.Double text_rect = new Rectangle2D.Double(midx(cur_bbox) - text_bound.height / 2,
	...
	static private Shape createCrossRingCleavage(double angle, double x, double y, double w, double h, int first_pos,
			int last_pos) {
	...
	private Shape createShape(Residue node, Rectangle par_bbox, Rectangle cur_bbox, Rectangle sup_bbox,
	...
	private Shape createRepetitionText(double angle, double x, double y, double w, double h, int min, int max) {
		...
		if (min >= 0 || max >= 0) {
			...
			ret.append(getTextShape(xmin, ymin, text, theGraphicOptions.LINKAGE_INFO_FONT_FACE,
		...
		if (min >= 0 || max >= 0) {
			...
			ret.append(getTextShape(xmax, ymax, text, theGraphicOptions.LINKAGE_INFO_FONT_FACE,
					theGraphicOptions.LINKAGE_INFO_SIZE), false);
		}
	...
	private Shape createTextShape(Residue node, Rectangle par_bbox, Rectangle cur_bbox, Rectangle sup_bbox,
			ResAngle orientation) {
		...
		if (shape.equals("endrep"))
			return createRepetitionText(orientation.getAngle(), x, y, w, h, node.getMinRepetitions(),
	...
	static private Shape createArc(double x, double y, double w, double h, int start_pos, int end_pos) {
		return new Arc2D.Double(x - 0.5 * w, y - 0.5 * h, 2 * w, 2 * h, -end_pos * 60. + 30.,
				-((start_pos - end_pos + 6) % 6) * 60., Arc2D.PIE);
	}
