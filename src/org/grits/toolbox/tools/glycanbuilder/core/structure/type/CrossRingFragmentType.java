...
public class CrossRingFragmentType extends ResidueType {
	...
	public CrossRingFragmentType(String init) throws Exception {
		...
		description = fragment_type + "_" + first_pos + "_" + last_pos + " cleavage of C" + anomeric_carbon + " "
				+ str_residues;
	...
	/**
	 * Return <code>true</code> if the ring fragment type can be applied to the
	 * given residue.
	 */
	public boolean matches(Residue r) {
		return (residues.contains(r.getTypeName()) && r.getAnomericCarbon() == anomeric_carbon
				&& r.getRingSize() == ring_size);
	}

	/**
	 * Return <code>true</code> if the ring fragment type matches the given
	 * information.
	 * 
	...
	/**
	 * Return the list of residue type identifiers to which this ring fragment can
	 * be applied.
	 */
	 ...
	 * ResidueType ret = new ResidueType(); ret.name = "#zcleavage"; ret.description
	 * = "Z Cleavage"; ret.superclass = "cleavage"; ret.can_redend = false;
	 * ret.can_parent = true; return ret; }
	 */
}