 ...
 * application. Information about cross ring fragment types is loaded at run
 * time from a configuration file. The dictionary is a singleton and all
...
public class CrossRingFragmentDictionary {
	 ...
	 * 
	 * @param fragment_type
	 *            type of cross ring fragment (A/B)
	 * @param first_pos
	 *            position of the first cleavage in the ring
	 * @param last_pos
	 *            position of the second cleavage in the ring
	 * @param r
	 *            residue for which the ring fragment is computed
	 */
	public static CrossRingFragmentType getCrossRingFragmentType(char fragment_type, int first_pos, int last_pos,
			Residue r) {
		for (CrossRingFragmentType crt : dictionary) {
	 ...
	 *            residue for which the ring fragment is computed
	 */
	public static Residue newCrossRingFragment(char fragment_type, int first_pos, int last_pos, Residue r)
			throws Exception {
		if (r == null)
		...
		if (type == null)
			throw new Exception("Invalid " + fragment_type + " cross-ring cleavage at positions " + first_pos + ","
					+ last_pos + " of residue " + r.getTypeName());
