...
public class GlycoCTParser implements GlycanParser {
	...
	private GlycoNode toSugar(Residue current, GlycoGraph nm_graph,
		...
		if (current.isStartRepetition()) {
			...
			//if
			if(bboxManager!=null && bboxManager.getBorder(current) != null){
	...
	private GlycoEdge toSugar(Linkage link) throws Exception {
		...
		for (Bond b : link.getBonds()) {
			...
			if ( nm_link.getParentLinkageType() == LinkageType.UNVALIDATED )
				nm_link.setParentLinkageType(LinkageType.H_AT_OH);
			if ( nm_link.getChildLinkageType() == LinkageType.UNVALIDATED )
				nm_link.setChildLinkageType(LinkageType.DEOXY);
